...
export function FilterControls({
    ...
    const applyFilters = (newSearchTerm: string, newFilterBy: string, newBooleanFilters: { [key: string]: boolean | undefined }, newView: string) => {
    ...
    const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        debounceTimeout.current = setTimeout(() => {
            applyFilters(newSearchTerm, filterBy, booleanFilters, currentView);
        }, 300); // 300ms debounce
    };
    ...
    const handleFilterByChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        applyFilters(searchTerm, newFilterBy, booleanFilters, currentView); // Apply immediately when filter property changes
    };

    const handleBooleanFilterChange = (key: string, value: boolean | undefined) => {
        const newBooleanFilters = { ...booleanFilters, [key]: value };
        setBooleanFilters(newBooleanFilters);
        applyFilters(searchTerm, filterBy, newBooleanFilters, currentView); // Apply immediately
    };

    const handleViewChange = (newView: string) => {
        setCurrentView(newView);
        applyFilters(searchTerm, filterBy, booleanFilters, newView); // Apply immediately
    };
